...
#   BUNDLE_ID="fun.workwork.captionanimator"
#   VERSION="0.1.0"
...
BUNDLE_ID="${BUNDLE_ID:-fun.workwork.captionanimator}"
...
$PYTHON_BIN - "$PAYLOAD_DIR/templates/index.html" "$APP_PUBLISHER" "$APP_AUTHOR" "$COPYRIGHT_YEAR" <<'PYHTML'
...
text = text.replace(
    "const logs = Array.isArray(data?.logs) ? data.logs.slice(-8) : [];",
    "const logs = Array.isArray(data?.logs) ? data.logs.slice(-30) : [];",
)

css_marker = r'''
    /* Packaged desktop additions: timeline-safe sidebar, release credits, and persistent logs. */
    :root {
      --trim-panel-reserved-height: 0px;
    }

    .project-sidebar {
      height: calc(100vh - var(--trim-panel-reserved-height));
...
'''
if "Packaged desktop additions: timeline-safe sidebar" not in text:
    text = text.replace("</style>", css_marker + "\n  </style>", 1)

js_marker = r'''
<script>
(() => {
  const releasePublisher = __PUBLISHER__;
  const releaseAuthor = __AUTHOR__;
  const releaseYear = __YEAR__;

  const trimPanelNode = document.getElementById('trimPanel');
  const syncTrimPanelReservation = () => {
    if (!trimPanelNode) return;
    const visible = trimPanelNode.classList.contains('show') && getComputedStyle(trimPanelNode).display !== 'none';
    const reserved = visible ? Math.ceil(trimPanelNode.getBoundingClientRect().height) + 8 : 0;
    document.documentElement.style.setProperty('--trim-panel-reserved-height', `${reserved}px`);
    document.body.classList.toggle('trim-panel-open', visible);
  };

  if (trimPanelNode) {
    new MutationObserver(syncTrimPanelReservation).observe(trimPanelNode, { attributes: true, attributeFilter: ['class', 'style'] });
    if ('ResizeObserver' in window) new ResizeObserver(syncTrimPanelReservation).observe(trimPanelNode);
    window.addEventListener('resize', syncTrimPanelReservation, { passive: true });
    requestAnimationFrame(syncTrimPanelReservation);
  }

  const sidebar = document.getElementById('projectSidebar');
  if (sidebar && !document.getElementById('appReleaseCredit')) {
    const credit = document.createElement('div');
    credit.id = 'appReleaseCredit';
    credit.className = 'app-release-credit';
    credit.innerHTML = `<strong>${releasePublisher}</strong><br>Created by ${releaseAuthor} · ${releaseYear}`;
    sidebar.appendChild(credit);
  }

  const headerActions = document.querySelector('.project-header-actions');
  if (!headerActions || document.getElementById('appLogsButton')) return;

  const logsButton = document.createElement('button');
  logsButton.id = 'appLogsButton';
  logsButton.type = 'button';
  logsButton.className = 'project-button';
  logsButton.textContent = 'Logs';
  logsButton.title = 'Open application logs';
  headerActions.prepend(logsButton);

  const overlay = document.createElement('div');
  overlay.id = 'appLogOverlay';
  overlay.className = 'app-log-overlay';
  overlay.innerHTML = `
    <section class="app-log-panel" role="dialog" aria-modal="true" aria-label="Application logs">
      <div class="app-log-header">
        <div>
          <div class="app-log-title">Application logs</div>
          <div id="appLogPath" class="app-log-path">Loading…</div>
        </div>
        <button id="appLogClose" type="button" class="project-button">Close</button>
      </div>
      <div class="app-log-actions">
        <button id="appLogRefresh" type="button" class="project-button">Refresh</button>
        <button id="appLogReveal" type="button" class="project-button">Reveal in Finder</button>
        <button id="appLogCopy" type="button" class="project-button">Copy</button>
      </div>
      <pre id="appLogOutput" class="app-log-output">Loading…</pre>
    </section>`;
  document.body.appendChild(overlay);

  const output = overlay.querySelector('#appLogOutput');
  const pathNode = overlay.querySelector('#appLogPath');

  async function refreshLogs() {
    output.textContent = 'Loading…';
    try {
      const response = await fetch('/api/app_logs', { cache: 'no-store' });
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.error || 'Unable to load logs.');
      pathNode.textContent = data.path || '';
      output.textContent = (data.lines || []).join('\n') || 'No log entries yet.';
      output.scrollTop = output.scrollHeight;
    } catch (error) {
      output.textContent = `Log error: ${error.message || error}`;
    }
  }
  ...
  overlay.querySelector('#appLogClose').addEventListener('click', closeLogs);
  overlay.querySelector('#appLogRefresh').addEventListener('click', refreshLogs);
  overlay.querySelector('#appLogReveal').addEventListener('click', async () => {
    await fetch('/api/reveal_app_logs', { method: 'POST' });
  });
  overlay.querySelector('#appLogCopy').addEventListener('click', async () => {
    await navigator.clipboard.writeText(output.textContent || '');
  });
  overlay.addEventListener('click', event => {
    if (event.target === overlay) closeLogs();
  });
  window.addEventListener('keydown', event => {
    if (event.key === 'Escape' && overlay.classList.contains('open')) closeLogs();
...
js_marker = (
    js_marker
    .replace("__PUBLISHER__", json.dumps(publisher))
    .replace("__AUTHOR__", json.dumps(author))
    .replace("__YEAR__", json.dumps(year))
)
if "id=\"appLogOverlay\"" not in text:
    text = text.replace("</body>", js_marker + "\n</body>", 1)
...
# Add persistent app logging that can be inspected from the packaged UI.
...
"$VENV_PYTHON" - "$BUILD_ROOT" "$APP_SAFE_NAME" "$APP_ICON_SOURCE" <<'PYICON'
from pathlib import Path
...
if source_value:
    ...
    image = ImageOps.contain(image, (900, 900), Image.Resampling.LANCZOS)
